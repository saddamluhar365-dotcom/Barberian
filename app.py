...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/events":
            run_id = parse_qs(parsed.query).get("run_id", [None])[0]; body = "".join(f"data: {json.dumps({'run_id': e.run_id, 'event_type': e.event_type, 'payload': e.payload, 'created_at': e.created_at})}\n\n" for e in EVENTS.replay(run_id)).encode(); self.send_response(200); self.send_header("Content-Type", "text/event-stream"); self.send_header("Cache-Control", "no-cache"); self.send_header("Content-Length", str(len(body))); self.end_headers(); self.wfile.write(body); return
    ...
    def do_POST(self):
        ...
        try:
            ...
            if size < 0 or size > 1_048_576: return self.send_json({"ok": False, "error": "request too large"}, 413)
            data = json.loads(self.rfile.read(size) or b"{}")
            if path == "/api/tasks":
                message = data.get("message", "")
                if not isinstance(message, str) or not message.strip(): return self.send_json({"ok": False, "error": "message is required"}, 400)
                task = QueueTask(str(uuid4()), {"message": message}); task_queue().enqueue(task); EVENTS.publish(ExecutionEvent(task.id, "queued", {"message": message})); return self.send_json({"ok": True, "task_id": task.id, "status": task.status}, 202)
